...
function displayFavoriteFoods(person) {
    for (let foodCategory in person) {
        let favoriteFoods = person[foodCategory];
        if (Array.isArray(favoriteFoods)) {
            favoriteFoods.forEach(food => console.log(food));
        } else if (typeof favoriteFoods === 'string') {
            console.log(favoriteFoods);
        } else if (typeof favoriteFoods === 'object') {
            for (let shakePlace in favoriteFoods[0]) {
                console.log(favoriteFoods[0][shakePlace]);
            }
        }
    }
}
